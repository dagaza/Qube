...
from core.llm_debug_sink import default_llm_debug_log_path  # noqa: E402
...
def main() -> int:
    ap = argparse.ArgumentParser(description="View Qube LLM debug log file.")
    ap.add_argument(
        "--last",
        type=int,
        default=200,
    ...
    path = args.path or default_llm_debug_log_path()
    ...
    # Follow mode: show last N then poll for growth
    pos = 0
    if path.is_file():
        lines = _read_tail_lines(path, last_n)
        lines = _filter_lines(lines, args.filter)

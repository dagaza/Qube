"""PsyArXiv preprint adapter via OSF Preprints API (no key)."""
...
logger = logging.getLogger("Qube.Knowledge.PsyArXiv")

ADAPTER_ID = "psyarxiv"
RETRIEVAL_METHOD = "psyarxiv_search"
...
def _row_from_preprint(item: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if not url and preprint_id:
        url = f"https://osf.io/preprints/psyarxiv/{preprint_id}/"
    if not url and doi:
...
def fetch_search_results(
    ...
    *,
    ...
    if _use_fixtures():
        fixture = _fixture_search_path("psyarxiv_search_cognitive_load.json")
        if fixture is not None:
            try:
                return json.loads(fixture.read_text(encoding="utf-8"))
            except Exception as exc:
                logger.warning("[PsyArXiv] fixture load failed: %s", exc)
    ...
    try:
        ...
        logger.warning("[PsyArXiv] budget exhausted; skipping retry")
        return {"data": []}
    except Exception as exc:
        logger.warning("[PsyArXiv] search failed: %s", exc)
        return {"data": []}
...
def search_psyarxiv(
    ...
    *,
    ...
    """Search PsyArXiv psychology preprints via OSF."""

...
def generate_composer_attachments_markdown() -> str:
    lines = [
        ...
        "",
        "The `@` palette has six categories: **Files**, **Conversations**, **Tools**, "
        "**Integrations**, **Skills**, and **Commands**. This page covers the **routing** "
        "attachment kinds that steer retrieval and context for a turn.",
        "",
        ...
        "",
        "Attach a **granted integration capability** from an MCP server configured under "
        "**Settings → Knowledge → Custom sources**. Browse **Integrations** in the `@` palette "
        "or search by namespace (for example `filesystem`).",
        "",
        "Example: `@[cap:mcp:filesystem/search-files] Find install notes in my Projects folder.`",
        "",
        "Capabilities require explicit permission under **Settings → Integrations** before invoke. "
        "See [Integrations settings](../features/settings/integrations.md) and "
        "[Connect an MCP server](../workflows/connect-mcp-server.md).",
        "",
        "My knowledge presets may alias several capability URNs as **`@[tool:user:…]`** — Qube "
        "resolves those bundles at invoke time.",
        "",
        "## Routing rule (order matters)",
        "",
        "You can insert several routing tokens, but only the **first** one in your message "
        "(left-to-right) controls behaviour — the first among `@[file:…]`, `@[chat:…]`, "
        "`@[tool:…]`, or `@[cap:…]`. Put the attachment you want **first**.",
        "",
        "Example: `@[tool:internet] @[file:doc.pdf]` uses web discovery, not the file.",
        "",
        "Example: `@[cap:mcp:filesystem/search-files] @[tool:library]` uses the integration "
        "capability, not Library search.",
        "",
...
def generate_composer_tools_markdown() -> str:
    ...
    lines = [
        ...
        "token among `@[file:…]`, `@[chat:…]`, `@[tool:…]`, or `@[cap:…]` in left-to-right order. "
        "See [Composer attachments](composer-attachments.md) for files, chats, capabilities, and mixing rules.",
        "",
        "## Built-in tools",
        "",
    ]
    ...
    lines.extend(
        [
            ...
            "They bundle selected Live Source adapters, web-fetch domains, and/or **integration "
            "capability URNs** — not Library folders.",
            "",
            ...
            "",
            "MCP servers configured under **Settings → Knowledge → Custom sources** expose "
            "**Integrations** palette entries as **`@[cap:mcp:namespace/action]`**. Permissions "
            "are managed under **Settings → Integrations**. Entries appear after **Test/Save** "
            "and grant review — they are not hard-coded in this reference.",
            "",
            "See [Integrations settings](../features/settings/integrations.md) and "
            "[Connect an MCP server](../workflows/connect-mcp-server.md).",
            "",
            ...
            "**Settings → Memory**.",
            "- **Integrations** (`@[cap:mcp:…]`) — configure MCP under **Settings → Knowledge → "
            "Custom sources**, grant under **Settings → Integrations**.",
            "",
            "## Also called",
            "",

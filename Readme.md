...
- **Self-reflection worker** — every 6 hours, batches 10 least-recently-reflected memories and asks the **CPU sidecar** (Qwen2-0.5B) to label each as `durable_user_fact` / `third_party_stub` / `system_claim` / `transient` / `unclear`. Anything other than `durable_user_fact` is marked `flagged_for_review` and surfaced in the Memory Manager's Flagged section. **Never auto-deletes** — final say belongs to you.

...
def _legacy_user_body_height(
    doc: "QTextDocument", wrap_w: int, text: str, fm: "QFontMetrics"
) -> int:
    """Previous blunt formula: max estimators + 14 px fudge (+ optional second max in sync)."""
    w = max(1, int(wrap_w))
...
class TestFontDescenderInset(unittest.TestCase):
    def test_descender_inset_is_smaller_than_legacy_line_fudge(self) -> None:
        _ensure_qapp()
        fm = QFontMetrics(QFont("Inter", 14))
        inset = font_descender_inset(fm)
        self.assertLess(inset, fm.lineSpacing())
        self.assertGreaterEqual(inset, fm.descent())


@unittest.skipUnless(_HAS_PYQT6, "PyQt6 not installed")
class TestMeasureWrappedBodyHeight(unittest.TestCase):
    @classmethod
    ...
    def test_single_line_plain_text(self) -> None:
        ...
        body = measure_wrapped_body_height(
            doc, wrap_w, min_body_px=float(fm.lineSpacing())
        )
        self.assertGreaterEqual(body, float(fm.lineSpacing()))
        self.assertLess(body, float(fm.lineSpacing()) * 2.0)

    def test_multiline_respects_block_stack(self) -> None:
        doc = QTextDocument()
        doc.setPlainText("Line one\nLine two\nLine three")
        wrap_w = 400
        body = measure_wrapped_body_height(doc, wrap_w)
        stack = document_block_stack_bottom(doc)
        self.assertGreaterEqual(body, stack)

    def test_markdown_heading_block_stack(self) -> None:
        doc = QTextDocument()
    ...
    def test_markdown_tight_height_not_inflated_by_document_size(self) -> None:
        doc = QTextDocument()
        doc.setDefaultStyleSheet(markdown_document_stylesheet(is_dark=True))
        ...
        self.assertGreaterEqual(tight, stack)
        if doc_h > stack + 4:
            self.assertLess(tight, doc_h)

    def test_streaming_height_covers_document_size(self) -> None:
        doc = QTextDocument()
        doc.setDefaultStyleSheet(markdown_document_stylesheet(is_dark=True))
        doc.setMarkdown("| col |\n| --- |\n| val |")
        wrap_w = 520
        ...
        doc.setTextWidth(float(wrap_w))
        doc_h = float(layout.documentSize().height()) if layout is not None else 0.0
        streaming = measure_markdown_body_height(
            doc, wrap_w, bottom_inset_px=2, streaming=True
        )
        tight = measure_markdown_body_height(doc, wrap_w, bottom_inset_px=2)
        self.assertGreaterEqual(streaming, doc_h)
        self.assertGreaterEqual(streaming, tight)


@unittest.skipUnless(_HAS_PYQT6, "PyQt6 not installed")
class TestUserBubbleHeightTightening(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        _ensure_qapp()

    def test_new_formula_saves_vertical_space_vs_legacy(self) -> None:
        doc = QTextDocument()
        ...
        self.assertGreaterEqual(old_h - new_h, 4)


@unittest.skipUnless(_HAS_PYQT6, "PyQt6 not installed")
class TestTextEditChromeVertical(unittest.TestCase):
    def test_chrome_includes_descender_not_magic_two(self) -> None:
        _ensure_qapp()
        fm = QFontMetrics(QFont("Inter", 14))
        inset = font_descender_inset(fm)
        ...
        self.assertEqual(chrome, inset)
        self.assertGreaterEqual(chrome, fm.descent())

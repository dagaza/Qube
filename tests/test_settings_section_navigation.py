...
class SettingsRegistryTests(unittest.TestCase):
    ...
    def test_seventeen_sections_registered(self) -> None:
        self.assertEqual(len(SETTINGS_SECTIONS), 17)

    def test_system_group_sections(self) -> None:
        system = [s for s in SETTINGS_SECTIONS if s.group == "System"]
        self.assertEqual(
            [s.id for s in system],
            ["system.backup", "privacy.data", "diagnostics", "advanced"],
        )
    ...
    def test_support_group_sections(self) -> None:
        ...
        self.assertEqual(
            [s.id for s in support],
            ["about", "license", "contact.feedback", "help"],
        )

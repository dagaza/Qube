...
def _with_models_root(fn):
    with tempfile.TemporaryDirectory() as tmp:
        models = Path(tmp) / "models"
        models.mkdir()
        with patch("core.auxiliary_cognition.models_root", return_value=models):
            fn(models)


def test_bundled_default_is_protected():
    def body(models: Path) -> None:
...
def test_resolve_falls_back_to_bundled_when_override_invalid():
    def body(models: Path) -> None:
        ...
        with patch(
            ...
            return_value=str(models.parent / "missing.gguf"),
        ):
